// script.js – Final Bulletproof Version with Error Handling
...
async function loadPublicStore(sitename) {
    ...
    document.body.innerHTML = `
        <div style="display: flex; flex-direction: column; align-items: center; justify-content: center; height: 100vh; font-family: sans-serif; background-color: #f8f9fa;">
            <div style="border: 4px solid #f3f3f3; border-top: 4px solid #0d6efd; border-radius: 50%; width: 50px; height: 50px; animation: spin 1s linear infinite; margin-bottom: 20px;"></div>
            <style>@keyframes spin { 0% { transform: rotate(0deg); } 100% { transform: rotate(360deg); } }</style>
            <h3>Loading Store "${sitename}"...</h3>
            <p style="color: #6c757d;">Connecting to database</p>
        </div>
    `;
    ...
    try {
        ...
        if (!targetUid) {
            document.body.innerHTML = `
                <div style="text-align: center; margin-top: 100px; font-family: sans-serif;">
                    <h1 style="font-size: 80px; color: #6c757d; margin-bottom: 0;">404</h1>
                    <h3 style="color: #343a40;">Store "${sitename}" not found.</h3>
                    <p style="color: #6c757d;">Please check the link and try again.</p>
                </div>
            `;
            return;
        }
        ...
        if (siteSnap.exists() && siteSnap.val()) {
            const htmlContent = siteSnap.val();
            document.open();
            document.write(htmlContent);
            document.close();
        } else {
            // Store exists but hasn't saved template yet
            document.body.innerHTML = `
                <div style="text-align: center; margin-top: 100px; font-family: sans-serif;">
                    <div style="display: inline-block; padding: 30px; background: #e0f8e9; border-radius: 10px; border: 1px solid #b2e8c6;">
                        <h3 style="color: #198754; margin-top: 0;">Store Setup Incomplete</h3>
                        <p style="color: #333;">This store has been created, but the design is not saved yet.</p>
                        <p style="color: #6c757d; font-size: 14px;">Store Owner: Please login, go to 'Website Builder' and click 'Save Changes'.</p>
                    </div>
                </div>
            `;
        }
        ...
        document.body.innerHTML = `
            <div style="text-align: center; margin-top: 100px; font-family: sans-serif;">
                <div style="display: inline-block; padding: 30px; background: #ffebeb; border-radius: 10px; border: 1px solid #ffc2c2; text-align: left; max-width: 500px;">
                    <h4 style="color: #dc3545; margin-top: 0;">Connection Blocked</h4>
                    <p style="color: #333;">Failed to load the store. This is usually because Firebase Security Rules are blocking access.</p>
                    <p style="color: #333;"><strong>Fix:</strong> Go to Firebase Console -> Database -> Rules, and set <code>".read": true</code>.</p>
                    <hr style="border-color: #ffc2c2;">
                    <p style="color: #dc3545; font-family: monospace; font-size: 12px; margin-bottom: 0;">Error log: ${error.message}</p>
                </div>
            </div>
        `;
    }
...
async function loadTemplateContent() {
    ...
    try {
        ...
        // Auto-fix: Save default if empty
        if (!savedHtml) {
